...
`STEAM_USERNAME` is also used for Workshop authentication when reusing a persisted `/home/steam/Steam` session cache.
`STEAM_PASSWORD` is only required when `TARGET_MANIFEST_ID` is set (version pinning).
